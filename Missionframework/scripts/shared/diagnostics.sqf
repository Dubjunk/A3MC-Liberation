...
while {true} do {
	if (isServer) then {
		diag_log format ["[KP LIBERATION] [STATS] Source: %1 - FPS: %2 - Total units: %3 - Hostile units: %4 - Local units: %5 - Vehicles: %6 - Active Sectors: %7 - All Groups: %8 - Local Groups: %9 - Empty Groups: %10",
		_source,
		...
		private _text = format ["[KP LIBERATION] [STATS] Source: %1 - FPS: %2 - Local units: %3 - Local Groups: %4",
		_source,
		((round (diag_fps * 100.0)) / 100.0),
		{local _x} count allUnits,
		{local _x} count allGroups];
