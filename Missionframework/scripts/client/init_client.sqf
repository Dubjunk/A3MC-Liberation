...
waitUntil {!isNil "KP_liberation_isSpawned"};
waitUntil {KP_liberation_isSpawned};

if !((getPlayerUID player) in KP_liberation_tfar_whitelist) then {
	[] spawn {
		while {true} do {

			if !(call TFAR_fnc_isTeamSpeakPluginEnabled) then {
				100 cutText ["TFAR ist deaktiviert. Aktiviere das TFAR Plugin in TeamSpeak!","BLACK"];
				waitUntil {sleep 0.5; call TFAR_fnc_isTeamSpeakPluginEnabled};
				100 cutText ["","PLAIN"];   
			};

			if (((call TFAR_fnc_getTeamSpeakServerName) != "[A3MC]>> ArmA3 MilSim - Community Deutschland <<[A3MC]") || ((call TFAR_fnc_getTeamSpeakChannelName) != "TFAR")) then {
				100 cutText ["Bitte verbinde dich zu unserem TS Server: ts3.a3m-community.de und lade dein TFAR Plugin anschließend neu.","BLACK"];
				waitUntil {sleep 0.5; (([] call TFAR_fnc_getTeamSpeakServerName) == "[A3MC]>> ArmA3 MilSim - Community Deutschland <<[A3MC]") && (([] call TFAR_fnc_getTeamSpeakChannelName) == "TFAR")};
				100 cutText ["","PLAIN"];
			};
